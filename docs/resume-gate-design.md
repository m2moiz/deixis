...
| `multiprocessing` in-process | On macOS the start method is **spawn**, so you re-import and reload the 2.4 GB model anyway — no saving over `Popen`. And it *loses* the coverage that motivates a subprocess gate at all: argparse wiring, `--status`, the piped-stderr bar branch, and a real SIGKILL that skips every `finally`, `atexit` and `BaseException` handler. The exception-based interrupt in `tests/test_resume_cli.py` already covers the in-process case, and it unwinds cleanly — which is exactly what a hard kill does not. |
