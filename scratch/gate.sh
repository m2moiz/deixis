...
uv run python -m deixis.transcribe scratch/meeting.wav -o scratch/gate_resumed.json --no-resume >scratch/gate_int.log 2>&1 &
PID=$!
sleep 45
...
echo "=== 3. resume ==="
uv run python -m deixis.transcribe scratch/meeting.wav -o scratch/gate_resumed.json 2>&1 | tail -2
