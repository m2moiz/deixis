...
uv run python -m dsj.suno scratch/meeting.wav -o scratch/gate_full.json --no-resume 2>&1 | tail -1

echo "=== 2. interrupted run (kill at ~45s) ==="
uv run python -m dsj.suno scratch/meeting.wav -o scratch/gate_resumed.json --no-resume >scratch/gate_int.log 2>&1 &
PID=$!
sleep 45
...
echo "=== 3. resume ==="
uv run python -m dsj.suno scratch/meeting.wav -o scratch/gate_resumed.json 2>&1 | tail -2
