"""Throwaway: re-run the §2.4 measurement through deixis.merge itself.
...
from deixis.merge import Turn, TurnIndex, label_sentence

HERE = Path(__file__).parent
TR = json.loads((HERE / "meeting_deixis.json").read_text())
DI = json.loads((HERE / "senko_meeting.json").read_text())

# Exactly what deixis/diarize.py's _to_turns does, inlined so this probe does
