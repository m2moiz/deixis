...
from deixis.transcribe import Progress, _make_chunk_callback
...
def test_chunk_callback_reports_seconds_not_samples():
    seen: list[Progress] = []
...
def test_chunk_callback_totals_stay_on_a_plausible_human_scale():
...
def test_chunk_callback_honours_the_models_sample_rate(rate):
...
def test_chunk_callback_stamps_elapsed_from_the_clock():
    seen: list[Progress] = []
    ticks = iter([2.0, 5.0])
...
def test_chunk_callback_emits_once_per_chunk():
    seen: list[Progress] = []
    cb = _make_chunk_callback(rate=RATE, clock=lambda: 1.0, emit=seen.append)
